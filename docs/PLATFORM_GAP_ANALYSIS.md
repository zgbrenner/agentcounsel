...
| Skill router | Implemented. `metadata/router.json` is generated from canonical skill metadata by `scripts/build_skill_index.py`, alongside the human/agent docs `WORKFLOW_ROUTER.md` and `COMMANDS.md`. | Closed. | None — keep the router generated from canonical metadata; do not hand-maintain a second router. |
...
| Legal prose quality pass | Implemented. `scripts/check_legal_prose.py` provides a stdlib automated pass over repository-owned sample outputs and eval candidates for AI tells, vague prose, overclaiming, and missing attorney-review framing, on top of the core output rules, skill checklists, and eval assertions. | Closed. | None — extend the checker's patterns as new failure modes are observed. |
