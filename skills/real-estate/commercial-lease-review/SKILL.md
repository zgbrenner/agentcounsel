...
**This skill does:** read a commercial lease from one stated party's
perspective; spot business and legal issues that perspective should weigh;
cite the clause, section, or page where each issue arises; organize the issues
into a risk matrix across the standard lease risk categories; describe the
direction in which a change would move risk; and produce a prioritized issue
list for attorney review.

**This skill does not:** give final advice or a recommendation; decide whether
the reviewing party should sign, refuse, or walk away from the lease;
determine whether any clause is enforceable; compute, confirm, or assume any
date or deadline; supply jurisdiction-specific law, recording rules, title or
zoning rules, or tax, securities, or financing requirements; or draft final
clause language. Those are attorney functions. Where the lease is silent or
unclear, the review says so — it does not fill the gap.
...
**Example request:** "Review this retail lease from the tenant's perspective —
we are a restaurant tenant negotiating a new lease in a shopping center. Here
is the full lease PDF and one exhibit."

**Expected output shape:** a header noting the property, the retail property
type, the new-lease posture, the tenant perspective, and `[CONFIRM: governing
law]` if the lease does not state it; a document-set list; a clause-by-clause
issue list working through rent and economic terms, use, exclusivity,
co-tenancy, go-dark, assignment and subletting, and the remaining categories,
each with a section or page citation and the risk to the tenant; a risk matrix
across those categories; a date-driven-obligations table with each date
flagged for verification; a prioritized issue list with a Suggested Direction
(not drafted language) for each High and Medium item; an open-items checklist;
and an assumptions list. No date is computed, no jurisdiction-specific law is
supplied, no final clause language is drafted, and the output does not tell
the tenant whether to sign.
