...
- **Consult `connectors/ecfr.md` and `connectors/federal-register.md`.** Where a connector is unavailable, the trace continues but every snapshot and every Federal Register reference carries the `not verified — no <X> connector available` flag. Where the history turns on the underlying enabling statute rather than the regulation itself, `connectors/congress-gov.md` can confirm that statute's or amendment's bill status and enactment record — existence and status only, never legal effect.
