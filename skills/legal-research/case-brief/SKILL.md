...
- **For a state-court opinion, also consult `connectors/state-courts.md` when a verification path is available** to confirm the citation form in a source that covers that state and record the verified URL — noting that state coverage is partial, so a not-found result proves nothing and good-law status is routed to `skills/legal-research/negative-treatment-check/SKILL.md`, not resolved here.
