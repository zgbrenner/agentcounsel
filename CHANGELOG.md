...
- Contracts references and skills upgraded with benchmark and negotiation-position material: `red-flags.md` extended to cover warranties, service levels, insurance, audit rights, publicity, non-solicit/non-compete, and governing law/forum; `negotiability-ratings.md` re-cast around a six-rating scale (Must Push, Strong Push, Business Call, Acceptable if Balanced, Low Priority, Do Not Spend Leverage) with stakeholder-communication language; `market-benchmark-framework.md` given a controlled characterization vocabulary (Common, Aggressive, Unusual, Depends on Leverage, Needs Attorney Confirmation); and a new `fallback-language-bank.md` of draft-only sample fallback positions. `contract-risk-review`, `nda-review`, and `sow-review` updated with Red Flags Quick Scan, Negotiability Table, Preferred / Fallback Position, Market Practice Notes, Suggested Redline Direction, and Business-Friendly Summary output sections.
